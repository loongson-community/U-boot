...
static unsigned char sys_ind_set[] = {0xc, 0x83, 0x0};
...
static char* partition_sys_ind_to_load(unsigned char sys_ind)
{
	int i;
	for (i = 0; ; ++i) {
		if (!sys_ind_set[i])
									...
									int last_enable, int* last_devid, int* last_partition)
{
	...
	for (partition = 0; partition < 64; ++partition) {
		...
		if (!ret && !partition_sys_ind_support(disk_info.sys_ind)) {
			cur_fs_type = partition_sys_ind_to_load(disk_info.sys_ind);
			ret = command_read_file(cur_fs_type, storage, desc->devnum, partition,
