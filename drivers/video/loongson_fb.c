...
static void config_pll(void __iomem *pll_base, struct ls_pll_cfg *pix_pll)
{
	...
	out = (pix_pll->l2_div << 24) | ((pix_pll->l1_loopc) << 16) |
				((pix_pll->l1_frefc) << 8);
	...
	writeq(0, pll_base);
	writeq(1 << 19, pll_base);	//power down pll first
	writeq(out, pll_base);
	writeq(pix_pll->l2_div, (ulong)pll_base + 0x8);
	out = (out | (1 << 2));
	writeq(out, pll_base);
	...
	// wait pll locked
	while (!(readq(pll_base) & (1 << 16))) ;

	writeq((out | 1), pll_base);
#endif
}
...
static int cal_freq(unsigned int pixclock_khz, struct ls_pll_cfg * pll_config)
{
	...
	printf("calculate PIX freq error!!!\n");
	return -EINVAL;
}
...
static int config_fb(struct ls_video_priv *priv, int dvo_id)
{
	...
	case PIX_FMT_RGB444:
		...
		ls_write_reg(base + LS_DC_FB_STRIDE, (mode.hactive * 2 + 255) & ~255);
		break;
	case PIX_FMT_RGB555:
	...
	case PIX_FMT_RGB888:
		...
		ls_write_reg(base + LS_DC_FB_STRIDE, (mode.hactive * 4 + 255) & ~255);
		break;
	default:
		ls_write_reg(base + LS_DC_FB_CONF, LS_FB_CONF_RESET | LS_FB_CONF_OUTPUT_EN | 0x4);
		ls_write_reg(base + LS_DC_FB_STRIDE, (mode.hactive * 4 + 255) & ~255);
...
static int ls_read_edid(struct ls_video_priv *priv, int block, u8 *buf)
{
	...
	if (priv->ddc_bus) {
		...
		if (ret) {
			printf("get ddc chip failed (err: %d)\n", ret);
			return ret;
		}
...
static int ls_video_get_display_timing(struct udevice *dev)
{
	...
	if (priv->conn_type == LS_CONNECTOR_PANEL) {
        ret = panel_get_display_timing(priv->connector, &priv->timing);
	} else {
...
failed:
    if (ret < 0) {
		printf("get display timing failed (err: %d), use the default timing.\n", ret);
...
static int ls_video_parse_dt(struct udevice *dev)
{
    ...
    if (ret || id >= LS_DC_MAX_DVO) {
		printf("%s: the value of reg prop is invalid\n", dev->name);
        return -EINVAL;
	}
	priv->id = id;
    ...
    if (priv->conn_type == LS_CONNECTOR_HDMI) {
        ret = uclass_get_device_by_phandle(UCLASS_DISPLAY, dev, "connector",
                    &priv->connector);
        if (ret) {
            printf("%s: Cannot find connector for '%s' (ret=%d)\n", __func__,
                dev->name, ret);
            priv->connector = NULL;
        }
        ...
        ret = uclass_get_device_by_phandle(UCLASS_PANEL, dev, "connector",
                        &priv->connector);
        if (ret) {
            printf("%s: Cannot find panel for '%s' (ret=%d)\n", __func__,
                dev->name, ret);
            return ret;
        }
...
static int ls_video_probe(struct udevice *dev)
{
	...
	debug("frame buffer addr: 0x%p\n", priv->fb_base);

    ret = ls_video_get_display_timing(dev);
    if (ret < 0) {
        printf("get display timing err: %d\n", ret);
		return ret;
    }
